...
export default class News extends React.Component {
	...
	render() {
		let {single, id, title, description, description_all, comments, author, date, rating, image, rate, unrate} = this.props;
		...
		let headerTemplate = (!single) ? (<h3 className="news-item-title"><Link to={"/news/" + id}>{title}</Link></h3>) : null,
			...
			descriptionTemplate1 = (!!single) ? (<p className="news-item-description">{description_all}</p>) : null,
